...
for(i in 1:m){
  ...
  mat[i,34] <- substr(auxFile, iSafetyStockB[[1]][1,2], iCompt[[1]][1,1]-3) 
  mat[i,35] <- substr(auxFile, iOptCosts[[1]][2,2], iTotalCosts[[1]][2,1]-3)
  mat[i,36] <- substr(auxFile, iTotalCosts[[1]][2,2], iDist[[1]][2,1]-3)
  mat[i,37] <- substr(auxFile, iDist[[1]][2,2], iTime[[1]][2,1]-3)
  mat[i,38] <- substr(auxFile, iTime[[1]][2,2], iFuelCost[[1]][2,1]-3)
  mat[i,39] <- substr(auxFile, iEconomicCost[[1]][2,2], iCo2Cost[[1]][2,1]-3)
  mat[i,40] <- substr(auxFile, iCo2Cost[[1]][2,2], iSocialCost[[1]][2,1]-3)  
  mat[i,41] <- substr(auxFile, iSocialCost[[1]][2,2], iCheckList[[1]][2,1]-3)
  mat[i,42] <- substr(auxFile, iExpOptCosts[[1]][2,2], iExpTotalCosts[[1]][2,1]-3)
  mat[i,43] <- substr(auxFile, iExpTotalCosts[[1]][2,2], iExpDist[[1]][2,1]-3)
  mat[i,44] <- substr(auxFile, iExpDist[[1]][2,2], iExpTime[[1]][2,1]-3)
  mat[i,45] <- substr(auxFile, iExpTime[[1]][2,2], iExpFuelCost[[1]][2,1]-3)
  mat[i,46] <- substr(auxFile, iExpEconomicCost[[1]][2,2], iExpCo2Cost[[1]][2,1]-3)
  mat[i,47] <- substr(auxFile, iExpCo2Cost[[1]][2,2], iExpSocialCost[[1]][2,1]-3)
  mat[i,48] <- substr(auxFile, iExpSocialCost[[1]][2,2], iExpDemand[[1]][2,1]-3)
  mat[i,49] <- substr(auxFile, iRelD[[1]][2,2], iRelT[[1]][2,1]-3)
  mat[i,50] <- substr(auxFile, iRelT[[1]][2,2], iRel[[1]][2,1]-3)
  mat[i,51] <- substr(auxFile, iRel[[1]][2,2], iSafetyStockC1[[1]][2,1]-3)
  mat[i,52] <- substr(auxFile, iCheckList[[1]][2,2], iStochInd[[1]][2,1]-3)
  mat[i,53] <- substr(auxFile, iSafetyStockC1[[1]][2,2], iSafetyStockC2[[1]][2,1]-3)
  mat[i,54] <- substr(auxFile, iSafetyStockC2[[1]][2,2], iSafetyStockB[[1]][2,1]-3)
  mat[i,55] <- substr(auxFile, iSafetyStockB[[1]][2,2], iCompt[[1]][2,1]-3) 
}
